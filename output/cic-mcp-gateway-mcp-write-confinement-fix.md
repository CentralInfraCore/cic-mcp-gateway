...
`PYTHONPATH` env forwarding — szerkezetét követve), **az orchestrátor által a
job-close review során javítva** `.venv-host/bin/python`-ra:
...
### Orchestrátor-korrekció: `p_venv` → `.venv-host` (job-close review során)

Az agent eredetileg `p_venv/bin/python`-t választotta, a `"cic-graph"` meglévő
bejegyzésével és a `mk/infra.mk` `PYTHON := ./p_venv/bin/python` változójával
konzisztensen, ÉS saját kézzel "megerősítette" hogy `p_venv/bin/python` ténylegesen
importálja a `gateway_server`-t. EZ A MEGERŐSÍTÉS FÉLREVEZETŐ volt: az agent a
SAJÁT, ad-hoc módon (feltehetően `python3 -m venv p_venv`-vel) épített tesztkörnyezetét
ellenőrizte, NEM azt, amit a repo TÉNYLEGES, dokumentált `make infra.deps` parancsa
épít. Az orchestrátor közvetlenül megnézte a `docker-compose.yml` `setup` service-ét:
...
`pip install --target` egy LAPOS package-dir-t épít, NINCS benne `bin/`
alkönyvtár — tehát egy FRISS clone, a dokumentált `make infra.deps`-et futtatva,
`p_venv/bin/python`-t SOHA nem kapna (ugyanaz a hiba, amit a `cic-mcp-shared`
repóban a `shared-cross-session-aggregator-implementation-001` jobnál az
orchestrátor korábban már önállóan bizonyított). A MEGLÉVŐ `"cic-graph"` bejegyzés
emiatt ELŐZETESEN, EZ A JOB ELŐTT is már törött volt — ezt ez a job NEM okozta és
NEM is ennek a jobnak a hatóköre javítani (lásd "Nem cél" az input.md-ben: a meglévő
`"cic-graph"` bejegyzés érintetlen kell maradjon).

Az ÚJ `"cic-gateway"` bejegyzésnél viszont, mivel a cél egy TÉNYLEGESEN használható
wiring, az orchestrátor `.venv-host/bin/python`-ra javította — ez a minta a
`gateway-context-pack-production-wiring-001` jobnál MÁR bizonyítottan működik
(az orchestrátor saját kézzel épített `.venv-host`-tal futtatta a valós
Postgres+subprocess teszteket). A `cic-mcp-gateway` repónak JELENLEG NINCS
`make deps.local`-ekvivalens target-je, ami ezt automatikusan felépítené (a
`cic-mcp-session` repóban VAN) — ez egy KÜLÖN, valós hiány, a "Next Jobs"-ban
javasolva, NEM ennek a jobnak a hatóköre megépíteni.

Megerősítve (az orchestrátor saját, kézzel épített `.venv-host`-jával, NEM a
repo dokumentált, törött `p_venv`-jével):

```
$ PYTHONPATH=. .venv-host/bin/python -c "
import gateway_core.compile_context
import sys
...
`gateway_server.py` ténylegesen `FastMCP("cic-gateway")`-t exponál, ÉS ez a
TÉNYLEGES, az új `.mcp.json.tpl` bejegyzésben hivatkozott `.venv-host`-tal
igazolt, nem egy ad-hoc, eltérő tesztkörnyezettel.
...
- Opcionális, KÜLÖN job: `record_decision()` `load_kb()`-mindig-hívás
  ...
  KB-betöltést).
- **(orchestrátor által hozzáadva a job-close review során)** `cic-mcp-gateway`-nek
  NINCS `make deps.local`-ekvivalens target-je, ami host-natívan felépítené a
  `.venv-host`-ot (a `cic-mcp-session` repóban VAN) — jelenleg ezt kézzel kell
  felépíteni (`python3 -m venv .venv-host && pip install -r requirements.txt`).
  Egy `deps.local`/`Makefile`/`mk/infra.mk` kiegészítő job lezárná ezt a hiányt.
- **(orchestrátor által hozzáadva)** a MEGLÉVŐ `"cic-graph"` bejegyzés
  `p_venv/bin/python`-ja ELŐZETESEN, ETTŐL A JOBTÓL FÜGGETLENÜL is törött (a
  `docker-compose.yml` `setup` service `pip install --target`-del épít, nincs
  `bin/`alkönyvtár) — ezt ez a job NEM javította (a "Nem cél" explicit tiltja a
  meglévő bejegyzés módosítását), de egy KÜLÖN job-ban érdemes lenne `cic-graph`-ot
  is `.venv-host`-ra migrálni, a `cic-mcp-session`-ben már elvégzett
  `session-mcp-venv-fix-report.md` mintáját követve.
